...
class ec_admin_orders{		
	...
	function updateorderaddresses($orderid, $addressinfo) {
		...
		//then, just get the new order information and return
		$sql = "SELECT ec_orderdetail.*, ec_order.*, ec_orderstatus.order_status, ec_orderstatus.is_approved, UNIX_TIMESTAMP(ec_order.order_date) AS order_date,  ec_download.date_created, ec_download.download_count FROM (((ec_order LEFT JOIN ec_orderdetail ON ec_orderdetail.order_id = ec_order.order_id) LEFT JOIN ec_download ON ec_orderdetail.download_key = ec_download.download_id) LEFT JOIN ec_orderstatus ON ec_order.orderstatus_id = ec_orderstatus.status_id) WHERE ec_order.order_id = %d ORDER BY ec_orderdetail.product_id";
