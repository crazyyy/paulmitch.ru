...
class ec_shipping{
	...
	private function get_live_based_radio( $i, $rate, $service_days = 0 ){
		...
		if( $rate != "ERROR" ){
			...
			if( $GLOBALS['ec_cart_data']->cart_data->shipping_method != "" && $GLOBALS['ec_cart_data']->cart_data->shipping_method == $this->live_based[$i][2] )
				$ret_string .= " checked=\"checked\"";
			else if( $GLOBALS['ec_cart_data']->cart_data->shipping_method == "" && $i == 0 ){
				$ret_string .= " checked=\"checked\"";
			}
	...
	private function get_live_based_select( $i, $rate ){
		...
		if( $rate != "ERROR" ){
			...
			if( ( $GLOBALS['ec_cart_data']->cart_data->shipping_method == "" && $i==0 ) || ( $GLOBALS['ec_cart_data']->cart_data->shipping_method != "" && $GLOBALS['ec_cart_data']->cart_data->shipping_method == $this->live_based[$i][0] ) )
			$ret_string .= " selected=\"selected\"";
		...
		if( $this->shipping_method == "price" ){
			...
			if( $rate == "ERROR" && $lowest_ship_method != "ERROR" ){
				...
				//if( isset( $this->destination_zip ) )
					//$GLOBALS['ec_cart_data']->cart_data->shipping_method = $lowest_ship_method;
			}
