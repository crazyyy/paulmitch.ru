...
class ec_product{
	...
	private function setup_product( $product_data ){
		...
		if( $GLOBALS['ec_cart_data']->cart_data->user_id != "" ){
			$roleprice = $this->mysqli->get_roleprice( $GLOBALS['ec_cart_data']->cart_data->user_id, $this->product_id );
			if( isset( $roleprice ) ){
	...
	public function display_price( ){
		...
		if( $this->list_price != "0.00" )
			...
			if( $this->pricing_per_sq_foot && !get_option( 'ec_option_enable_metric_unit_display' ) ){ echo "/sq ft"; }
			else if( $this->pricing_per_sq_foot && get_option( 'ec_option_enable_metric_unit_display' ) ){ echo "/sq m"; }
	...
	public function display_product_list_price( ){
		...
		if( $this->list_price != "0.000" ){
			echo "<span class=\"ec_product_old_price\">";
			if( $GLOBALS['currency']->symbol_location ){ 
				echo "<span class=\"currency\">" . $GLOBALS['currency']->get_symbol( ) . "</span>";
			}
			echo "<span class=\"dollar\">" . $p_dollar . "</span><span class=\"cent\">." . $p_cent . "</span>";
			if( !$GLOBALS['currency']->symbol_location ){ 
				echo "<span class=\"currency\">" . $GLOBALS['currency']->get_symbol( ) . "</span>";
			}
			echo "</span>";
