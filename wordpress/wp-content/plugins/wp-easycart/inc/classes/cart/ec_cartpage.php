...
class ec_cartpage{
	...
	function __construct( $is_affirm = false ){
		...
		$this->tax = new ec_tax( $this->cart->subtotal, $this->cart->taxable_subtotal - $sales_tax_discount->coupon_discount, 0, $GLOBALS['ec_user']->shipping->state, $GLOBALS['ec_user']->shipping->country, $GLOBALS['ec_user']->taxfree, $this->shipping->get_shipping_price( ) );
		...
		$this->tax = new ec_tax( $this->cart->subtotal, $this->cart->taxable_subtotal - $sales_tax_discount->coupon_discount, $vatable_subtotal, $GLOBALS['ec_user']->shipping->state, $GLOBALS['ec_user']->shipping->country, $GLOBALS['ec_user']->taxfree, $this->shipping->get_shipping_price( ) );
		...
		if( isset( $GLOBALS['ec_cart_data']->cart_data->subscription_advanced_option ) && $GLOBALS['ec_cart_data']->cart_data->subscription_advanced_option != "" )
			$this->subscription_advanced_options = json_decode( $GLOBALS['ec_cart_data']->cart_data->subscription_advanced_option, true );
		else
	...
	public function display_estimate_shipping_country_select( ){
		if( $GLOBALS['ec_cart_data']->cart_data->estimate_shipping_country != "" )		$selected_country = $GLOBALS['ec_cart_data']->cart_data->estimate_shipping_country;
				...
				if( count( $subscription_list ) <= 0 ){
					...
					if( class_exists( "ec_stripe" ) ){
						...
						if( $customer_insert_test ){ // Customer inserted successfully (OR didn't need to be inserted)
							...
							if( $card_result ){ //Card Submitted Successfully
								...
								if( $plan_added ){ // Plan added successfully
									...
									$stripe_response = $stripe->insert_subscription( $product, $GLOBALS['ec_user'], $card, $coupon, $quantity );
						...
						*/
						...
						$GLOBALS['ec_cart_data']->ec_cart_id = $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)] . $vals[rand(0, 25)];
						setcookie( 'ec_cart_id', $GLOBALS['ec_cart_data']->ec_cart_id, time( ) + ( 3600 * 24 * 1 ), "/" );
	...
	public function process_subscribe_v3( ){
		...
		$GLOBALS['ec_cart_data']->cart_data->subscription_advanced_option = json_encode( $option_vals );
		$GLOBALS['ec_cart_data']->save_session_to_db( );
