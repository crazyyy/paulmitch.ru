...
add_action( 'wpec_tgmpa_register', 'wp_easycart_register_add_on_plugins' );
...
function ec_install_admin_notice() {
	if( current_user_can( 'manage_options' ) ){
		...
		if( is_plugin_active( "wp-easycart-admin/wpeasycart-admin.php" ) && version_compare( EC_AD_CURRENT_VERSION, "3.0.23" ) < 0 ){
			?>
			<div class="error">
...
function wp_easycart_register_add_on_plugins( ){
	...
	$plugins = array(
		...
		array(
			...
			'source'       => 'http://wpeasycart.com/air/wp-easycart-admin.zip', // The plugin source.
