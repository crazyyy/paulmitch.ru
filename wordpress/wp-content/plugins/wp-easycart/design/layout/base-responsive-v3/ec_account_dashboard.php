    ...
    <div class="ec_account_left">
		...
		<div class="ec_account_order_header_row<?php if( !$is_first_order ){?> ec_account_order_header_row_not_first <?php } $is_first_order = false; ?>">
        	<div class="ec_account_order_header_column_left ec_account_order_header_column_left_div1">
            	<span><?php echo $GLOBALS['language']->get_text( 'account_dashboard', 'account_dashboard_order_placed' )?></span>
            ...
            <div class="ec_account_order_header_column_left ec_account_order_header_column_left_div3">
                ...
                <div>
                	<a href="#" class="ec_account_dashboard_order_info_link"><?php $order->display_order_shipping_first_name( ); ?> <?php $order->display_order_shipping_last_name( ); ?>
                        ...
                        <?php $order->display_order_shipping_country( ); ?><br />
                        <?php echo $GLOBALS['language']->get_text( 'account_billing_information', 'account_billing_information_phone' )?>: <?php $order->display_order_shipping_phone( ); ?></span>
                    </a>
                </div>
            </div>
        ...
        <div class="ec_account_order_item_row">
					...
					if( $order_item->use_advanced_optionset ){
						...
						foreach( $advanced_options as $advanced_option ){
							...
							if( $advanced_option->option_type == "file" ){
								...
								echo "<span>" . $advanced_option->option_name . ":</span> <span class=\"ec_option_name\">" . $file_split[1] . $advanced_option->option_price_change . "</span>";
				
							}else if( $advanced_option->option_type == "grid" ){
								echo "<span>" . $advanced_option->option_name . ":</span> <span class=\"ec_option_name\">" . $advanced_option->optionitem_name . " (" . $advanced_option->option_value . ")" . $advanced_option->option_price_change . "</span>";
				
							}else{
								echo "<span>" . $advanced_option->option_name . ": " . htmlspecialchars( $advanced_option->option_value, ENT_QUOTES ) . $advanced_option->option_price_change . "</span>";
            ...
            <div class="ec_account_order_item_buttons<?php if( $product_link && $order_item->has_download_link( ) && $order->is_approved && $advanced_optionitem_download_allowed ){ echo " ec_account_order_item_two_buttons"; } ?>">
                ...
                <?php if( $order_item->has_download_link( ) && $order->is_approved && $advanced_optionitem_download_allowed ){
					 ...
					 if( $order_item->maximum_downloads_allowed > 0 ){
						 echo "<span id=\"ec_download_count_" . $order_item->orderdetail_id . "\">" . $order_item->download_count . "</span>" . "/" . "<span id=\"ec_download_count_max_" . $order_item->orderdetail_id . "\">" . $order_item->maximum_downloads_allowed . "</span> " . $GLOBALS['language']->get_text( 'account_order_details', 'account_orders_details_downloads_used' );
				
					 }
					 
					 if( $order_item->download_timelimit_seconds > 0 ){
						 echo "<span>";
						 echo $GLOBALS['language']->get_text( 'account_order_details', 'account_orders_details_downloads_expire_time' ) . " " . $order_item->get_download_expire_date( "d M Y" );
						 echo "</span>";
					 }
				...
				if( $product_link ){ ?>
                <span class="ec_account_order_item_buy_button"><a href="<?php echo $product_link; ?>" target="_blank"><?php echo $GLOBALS['language']->get_text( 'account_dashboard', 'account_dashboard_order_buy_item_again' )?></a></span>
            	<?php }?>
            </div>
        </div>
        <?php } ?>
