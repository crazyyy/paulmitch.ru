...
define( 'EC_CURRENT_VERSION', '3_2_1' );
define( 'EC_CURRENT_DB', '1_30' );
define( 'EC_UPGRADE_DB', '37' );
...
function load_ec_pre(){
	...
	if( !get_option( 'ec_option_new_linking_setup' ) ){
		...
		foreach( $product_list as $product_single ){
			if( isset( $product_single->post_id ) && $product_single->post_id == 0 ){
				// Add a post id
...
function ec_get_order_totals( ){
	...
	$cart = new ec_cart( $GLOBALS['ec_cart_data']->ec_cart_id );
	$shipping = new ec_shipping( $cart->shipping_subtotal, $cart->weight, $cart->shippable_total_items, 'RADIO', $GLOBALS['ec_user']->freeshipping );
	$sales_tax_discount = new ec_discount( $cart, $cart->subtotal, 0.00, $coupon_code, "", 0 );
	$tax = new ec_tax( $cart->subtotal, $cart->taxable_subtotal - $sales_tax_discount->coupon_discount, 0, $GLOBALS['ec_user']->shipping->state, $GLOBALS['ec_user']->shipping->country, $GLOBALS['ec_user']->taxfree, $shipping->get_shipping_price( ) );
	...
	if( get_option( 'ec_option_no_vat_on_shipping' ) )
		$vatable_subtotal = $vatable_subtotal - $shipping->get_shipping_price( );
	$tax = new ec_tax( $cart->subtotal, $cart->taxable_subtotal - $sales_tax_discount->coupon_discount, $vatable_subtotal, $GLOBALS['ec_user']->shipping->state, $GLOBALS['ec_user']->shipping->country, $GLOBALS['ec_user']->taxfree, $shipping->get_shipping_price( ) );
